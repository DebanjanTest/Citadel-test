...
const Header: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 left-0 w-full z-[60] px-8 md:px-16 py-8 flex justify-between items-center bg-transparent backdrop-blur-sm lg:backdrop-blur-none transition-all">
      ...
      <div className={`fixed inset-0 bg-[#050505] z-40 transition-transform duration-500 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-full'} flex flex-col items-center justify-center space-y-10 lg:hidden`}>
        ...
        <button className="mt-6 px-12 py-4 border-2 border-amber-600 text-amber-500 font-bold font-pyriform text-xl tracking-widest hover:bg-amber-600 hover:text-black transition-all">
          ASCEND
        </button>
      </div>
    </header>
